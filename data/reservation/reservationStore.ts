import { IAjaxConnector, AjaxConnector, StaticConnector } from "../ajaxUtils";
import { IReservation, IReservationListRequest, IReservationListResponse, IReservationStore } from "./types";
...
class ReservationStore implements ReservationStore {
    ...
    processReservationListResponse(response: IReservationListResponse | undefined): void {
...
export function reservationStoreFactory(): IReservationStore {
    ...
    if(debug) {
        connector = new StaticConnector(reservationListDataMock, store.processReservationListResponse);
    } else {
        connector = new AjaxConnector("GET", store.getUrl, store.processReservationListResponse);
    }
